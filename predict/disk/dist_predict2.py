...
def sl_by_dataframe():
    out_folder = 'E:\\mldata\\dataframe'
    rdd_name = os.path.join(out_folder, file_name)

    if os.path.exists(rdd_name):
        # rdd = mytest.textFile(rdd_name).map(lambda p: Row(
        #     failure=p[0],
        #     indexedFeatures=p[1]
        # ))
        # data = sql_context.createDataFrame(rdd)
        data = sql_context.read.format("parquet").load(rdd_name)
    else:
        data = read_csv(spark, os.path.join(in_folder, file_name))
        print("read one file use time:" + str(time.time() - exec_start_time))
        # rdd = data.select("failure", "indexedFeatures").rdd
        # rdd.repartition(
        #     1).saveAsTextFile(
        #     rdd_name)
        data.select("failure", "indexedFeatures").write.format("parquet").save(rdd_name)
